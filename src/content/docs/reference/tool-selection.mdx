...
- MCP 관련 실습(Lab 03)은 **Claude Code, Gemini CLI, Codex CLI**에서 수행 가능하다.
- Codex는 `~/.codex/config.toml` 또는 프로젝트 `.codex/config.toml`에 MCP 서버를 등록한다. TUI에서는 `/mcp`로 활성 서버를 확인한다.
