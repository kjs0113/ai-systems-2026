...
<Aside type="tip" title="L8 mapping — what this code is an instance of">
This code is effectively an informal implementation of two concepts from Agent OS Runtime's [L8 Workflow Plane](/en/reference/agent-os-runtime/#workflow-plane-l8): invoking the Correctness / Quality / Architecture reviewers in parallel and merging their results is **persona fan-out / fan-in**, and the PASS/FAIL gate driven by severity × score is a combination of **severity-routing** and **confidence-gating** policies. The lab's `critical/major/minor/info` severities can map to `P0/P1/P2/P3` in L8 `finding_v1`. If you move this to the L8 SSOT (`workflows/personas/`, `workflows/policies/`), you can add a new reviewer dimension or adjust severity thresholds with zero code lines, and every decision is captured as a `workflow.policy_gated` event in the audit log.
</Aside>
...
<div class="artifact-chain diagram">
  <div class="diagram__title">ARTIFACT CHAIN</div>
  <div class="artifact-chain__layout">
    <div class="artifact-chain__flow">
      <div class="artifact-chain__item artifact-chain__item--blue">
        <span class="artifact-chain__meta">Planner input</span>
        <code>requirement.md</code>
      </div>
      <div class="artifact-chain__arrow" aria-hidden="true">↓</div>
      <div class="artifact-chain__item artifact-chain__item--purple">
        <span class="artifact-chain__meta">Planner output · Week 8</span>
        <code>architecture.md</code>
      </div>
      <div class="artifact-chain__arrow" aria-hidden="true">↓</div>
      <div class="artifact-chain__item artifact-chain__item--cyan">
        <span class="artifact-chain__meta">Coder assignment from task_queue.json</span>
        <code>TASK-001.md · TASK-002.md</code>
      </div>
      <div class="artifact-chain__arrow" aria-hidden="true">↓</div>
      <div class="artifact-chain__item artifact-chain__item--orange">
        <span class="artifact-chain__meta">Coder output</span>
        <code>PR (code + tests)</code>
      </div>
      <div class="artifact-chain__arrow" aria-hidden="true">↓</div>
      <div class="artifact-chain__item artifact-chain__item--green">
        <span class="artifact-chain__meta">QA output · Week 9</span>
        <code>review-results.json</code>
      </div>
      <div class="artifact-chain__arrow" aria-hidden="true">↓</div>
      <div class="artifact-chain__item artifact-chain__item--gray">
        <span class="artifact-chain__meta">Record of learned failure patterns</span>
        <code>LESSON-001.md</code>
      </div>
    </div>
    <div class="artifact-chain__state">
      <span class="artifact-chain__state-label">Central State Store</span>
      <code>pipeline-state.json</code>
      <p>Records each artifact's creation time, responsible agent, and current Phase so the pipeline can restart from the interruption point.</p>
    </div>
  </div>
</div>
