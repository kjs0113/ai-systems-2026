...
<CardGrid>
  <Card title="Protocol Perspective" icon="external">
  ...
  <Card title="Architecture Perspective" icon="seti:db">
  ...
  <Card title="Implementation Perspective" icon="pencil">
...
The latest MCP spec (2025-11-25) states explicitly: **tool descriptions from untrusted servers must also be treated as untrusted**. Tool descriptions themselves can contain embedded prompt injections. This is the core attack vector of the McpInject attack covered in the "Security and Governance" section below.
</Aside>
...
<Aside type="note" title="Agent OS Runtime perspective">
This week, MCP should be understood as a **capability boundary**, not just a way to attach tools. In Agent OS Runtime, every tool call passes through `allowed-tools` scope, input/output schemas, `PreToolUse`/`PostToolUse` hooks, and `tool.invoke`/`tool.result` events. This becomes L1 in the [Agent OS Runtime core architecture](/en/reference/agent-os-runtime/#7-layer-architecture).
</Aside>
...
- [OpenAI MCP Guide](https://platform.openai.com/docs/guides) — Cross-vendor adoption evidence
