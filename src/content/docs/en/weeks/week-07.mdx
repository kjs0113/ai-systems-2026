...
In Agent OS Runtime's [L8 Workflow Plane](/en/reference/agent-os-runtime/#workflow-plane-l8) vocabulary, this 9-stage gated pipeline is one instance of a "ship-cycle" — each stage is a **phase**, each `/validate` gate is a **policy** (`severity-routing` + `loop-halt`), the coder/reviewer/QA roles are **personas**, and `spec.md` / TASK files are **artifacts**. The "3 retries then human escalation" rule is an instance of the `loop-halt` policy. Once you know the L8 vocabulary, you can design other pipelines for the capstone — brainstorm-cycle, fix-cycle, ralph-cycle — inside the same frame.
</Aside>
