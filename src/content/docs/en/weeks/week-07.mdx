...
<Aside type="note" title="Preview: L8 Workflow Plane">
In Agent OS Runtime's [L8 Workflow Plane](/en/reference/agent-os-runtime/#workflow-plane-l8) vocabulary, this 9-stage gated pipeline is one instance of a "ship-cycle" — each stage is a **phase**, validation / approval / retry rules are **policies**, the coder / reviewer / QA roles are **personas**, and `spec.md` / TASK files are **artifacts**. For example, routing by review severity is `severity-routing`, while "3 retries then human escalation" is a `loop-halt` policy. Once you know the L8 vocabulary, you can design other pipelines for the capstone — brainstorm-cycle, fix-cycle, ralph-cycle — inside the same frame.
</Aside>
...
The default handoff pattern for this week is Agent OS Runtime's **Lead → Planner → Worker → Reviewer** flow. Lead emits `lead_directive_v1`, Planner emits `plan_v1`, Worker emits `worker_report_v1`, and Reviewer emits `review_result_v1`. The full contracts and role prompts are self-contained in [Agent OS Contracts](/en/reference/agent-os-runtime/#contracts).
</Aside>
...
**Hallucination propagation** is the hardest to detect. If the Planner specifies a non-existent API in the spec, the Architect accepts it as fact and decomposes tasks accordingly, and the Coder writes code that calls that API. **Each agent operated correctly within its own stage**, yet the final output is wrong. This is the fundamental reason why **independent validation gates at each phase are necessary**.
</Aside>
