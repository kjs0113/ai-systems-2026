...
OpenTelemetry spans answer "what is slow right now." Agent OS Runtime `.events.jsonl` answers "what exactly happened in this run." Week 12 uses both: OTel for live monitoring, and an [append-only event store](/en/reference/agent-os-runtime/#7-layer-architecture) for audit and replay. Teams using the L8 workflow plane also prove `workflow.started`, `workflow.phase_advanced`, and `workflow.policy_gated` events plus replay of the current cycle, phase, and verdict.
</Aside>
