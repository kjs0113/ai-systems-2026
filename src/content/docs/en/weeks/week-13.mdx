...
> See [Agent OS 7+1-Layer Architecture (L1–L7 core + L8 workflow plane)](/en/reference/agent-os-runtime/#7-layer-architecture) for layer-by-layer detail. Teams that want to design multi-phase cycles (e.g. brainstorm → fix → ship) should also consult the [L8 Workflow Plane](/en/reference/agent-os-runtime/#workflow-plane-l8) five-axis model (cycle / phase / policy / persona / artifact). L8 is **optional** — the capstone rubric is fully satisfied by the L1–L7 core alone; L8 is for teams that want to lift cycle sequencing, policies, and personas into Markdown SSOT.
