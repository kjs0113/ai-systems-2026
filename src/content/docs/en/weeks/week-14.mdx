...
Lead → Planner → Worker → Reviewer can adopt the [Agent OS Contracts](/en/reference/agent-os-runtime/#contracts) `lead_directive_v1`, `plan_v1`, `worker_report_v1`, `review_result_v1` shapes as self-contained schemas. Teams that run multi-phase cycles (brainstorm → fix → ship etc.) may additionally submit [L8 Workflow Plane](/en/reference/agent-os-runtime/#workflow-plane-l8) `cycle_v1` / `phase_v1` / `policy_v1` Markdown SSOT — optional, with rubric credit.
</Aside>
