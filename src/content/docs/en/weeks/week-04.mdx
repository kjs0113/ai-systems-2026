...
### Part 6: Practical Harness Optimization — Parallel Sessions and Worktree Isolation
...
#### 6.1 Parallel Sessions — Developer-Level Multiplexing
...
The key is that **each instance handles an independent task**. There is no inter-agent communication. Git is the sole coordination mechanism — a natural extension of the Ralph loop's monolithic philosophy (Part 2).
...
#### 6.2 The `--worktree` Flag — Native Isolation
...
#### 6.3 `/sandbox` — Trust-Based Isolation
...
#### 6.4 Stack Philosophy — Layers Build on Each Other
...
The Ralph loop harness from Part 2 (backpressure + GC + AGENTS.md) ensures **quality for a single loop**. The Boris stack builds on top of it to add **throughput and isolation across loops**. Ralph answers "how do I run one loop well?"; Boris answers "how do I safely run ten loops at once?"
</Aside>

#### 6.5 Agentmaxxing — Multi-Tool Parallel Execution
...
### Part 7: Real-World Validation — Building a Product Without Writing a Line of Code
