...
<Aside type="note" title="Markdown-SSOT Skill Runtime">
Agent OS Runtime generalizes CLAUDE.md/PROMPT.md into `SKILL.md`. Skill frontmatter declares `triggers`, `allowed-tools`, and `schema_version`; the runtime validates it with `skill_frontmatter_v1` and scopes the available tools. Instruction tuning therefore becomes executable as [Markdown-SSOT plus hook-gated policy](/en/reference/agent-os-runtime/#contracts).
</Aside>
