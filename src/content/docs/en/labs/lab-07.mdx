...
The minimum shared contract for this lab is Agent OS Runtime's `lead_directive_v1`, `plan_v1`, `worker_report_v1`, and `review_result_v1`. The `planner_output.json` and `coder_output.json` below are domain-specific extensions. Submissions must include the minimum shared contracts as well. Use [Agent OS Contracts](/en/reference/agent-os-runtime/#contracts) as the canonical source.
...
- [ ] `sessions/example/.events.jsonl` or equivalent execution trace
- [ ] `replay_snapshot.json` — final state derived from the execution trace
