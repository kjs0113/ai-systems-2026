...
2. **Install vLLM**
   ...
   # If uv is unavailable: use the CUDA 12.9 wheel index
...
<Tabs>
  <TabItem label="Claude Code">
    ```bash
    # Claude Code assumes the Anthropic API by default.
    # If your installed version supports a local OpenAI-compatible endpoint, document the exact config in README.md.
    ```
  </TabItem>
  <TabItem label="Gemini CLI">
    ```bash
    # OpenAI-compatible endpoint support varies by Gemini CLI version.
    # The required verification path is local_agent.py below.
    ```
  </TabItem>
  <TabItem label="Codex CLI">
    ```bash
    # For Codex CLI, confirm the provider/base_url settings supported by your installed version.
    # Direct local-server calls are verified with local_agent.py.
    ```
  </TabItem>
