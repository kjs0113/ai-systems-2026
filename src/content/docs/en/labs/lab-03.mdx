...
- **Codex CLI**: Experimental support (`~/.codex/config.toml`) — required validation for this lab still uses Claude Code or Gemini CLI
...
<Tabs>
  ...
  <TabItem label="Codex CLI">
    Config file: `~/.codex/config.toml`. Codex MCP support varies by version, so first confirm registration with `codex mcp list`.
  </TabItem>
</Tabs>
