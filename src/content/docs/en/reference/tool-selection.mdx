...
| **MCP Support** | Full | Supported | Supported | Limited |
...
- MCP-related labs (Lab 03) can be completed with **Claude Code, Gemini CLI, or Codex CLI**.
- Codex registers MCP servers in `~/.codex/config.toml` or project-scoped `.codex/config.toml`. Use `/mcp` in the TUI to inspect active servers.
