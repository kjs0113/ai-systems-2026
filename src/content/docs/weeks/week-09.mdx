...
DeepMind와 MIT의 에이전트 시스템 스케일링 연구에 따르면, **검증 에이전트가 생성 에이전트와 컨텍스트를 공유할 경우 동일한 편향을 상속**한다. 코더가 특정 가정 하에 코드를 작성했다면, 같은 컨텍스트를 가진 QA 에이전트는 그 가정을 전제로 삼아 검증을 생략할 수 있다. 이는 공유 컨텍스트가 검증 독립성을 약화시키는 사례다.
...
<Aside type="tip" title="L8 매핑 — 이 구현이 무엇의 인스턴스인가">
이 코드는 사실상 Agent OS Runtime [L8 Workflow Plane](/reference/agent-os-runtime/#workflow-plane-l8)의 두 개념을 비공식으로 구현한 것이다 — 정확성/품질/아키텍처 리뷰어를 동시에 호출하고 결과를 합치는 부분은 **persona fan-out / fan-in**, 심각도 × 점수 기준으로 PASS/FAIL을 결정하는 게이트는 **severity-routing + confidence-gating** 정책이다. 이 실습의 `critical/major/minor/info`는 L8 `finding_v1`의 `P0/P1/P2/P3`로 매핑할 수 있다. L8 SSOT(`workflows/personas/`, `workflows/policies/`)로 옮기면 코드 0줄로 새 리뷰어 차원을 추가하거나 심각도 기준을 조정할 수 있고, 결정마다 `workflow.policy_gated` event가 audit log에 남는다.
</Aside>
  ...
  <div class="artifact-chain__layout">
    <div class="artifact-chain__flow">
      <div class="artifact-chain__item artifact-chain__item--blue">
        <span class="artifact-chain__meta">Planner 입력</span>
    ...
    <div class="artifact-chain__state">
      ...
      <p>각 아티팩트의 생성 시점, 담당 에이전트, 현재 Phase를 기록하고 중단 지점부터 재시작할 수 있게 한다.</p>
    </div>
  </div>
</div>
