...
OpenTelemetry span은 "지금 무엇이 느린가"를 보여주고, Agent OS Runtime의 `.events.jsonl`은 "그 세션에서 실제로 무엇이 일어났는가"를 재구성한다. Week 12의 관측성은 두 축을 함께 다룬다: live monitoring은 OTel, audit/replay는 [append-only event store](/reference/agent-os-runtime/#7-layer-architecture)가 담당한다. L8 workflow plane을 쓰는 팀은 `workflow.started`, `workflow.phase_advanced`, `workflow.policy_gated` event와 current cycle/phase/verdict replay까지 함께 증명한다.
</Aside>
