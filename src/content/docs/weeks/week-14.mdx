...
L8은 선택 확장이다. 다중 phase 사이클(brainstorm → fix → ship 등)을 직접 굴리는 팀만 추가로 `workflows/cycles/`, `workflows/phases/`, `workflows/policies/` Markdown SSOT와 `workflow.started` / `workflow.phase_advanced` / `workflow.policy_gated` event 증거를 제출한다.
</Aside>
