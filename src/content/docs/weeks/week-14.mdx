...
Lead → Planner → Worker → Reviewer 계약은 [Agent OS Contracts](/reference/agent-os-runtime/#contracts)의 `lead_directive_v1`, `plan_v1`, `worker_report_v1`, `review_result_v1` 구조를 참고해 self-contained로 작성한다. 다중 phase 사이클(brainstorm → fix → ship 등)을 직접 굴리는 팀은 추가로 [L8 Workflow Plane](/reference/agent-os-runtime/#workflow-plane-l8)의 `cycle_v1`/`phase_v1`/`policy_v1` markdown SSOT를 함께 제출할 수 있다 — 옵션이며 평가 가산.
</Aside>
