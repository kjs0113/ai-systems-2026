...
MCP 규격(2025-11-25)은 명시한다: **신뢰할 수 없는 서버의 도구 설명(description)도 untrusted로 취급**해야 한다. 도구 설명 자체에 프롬프트 인젝션이 내장될 수 있기 때문이다. 이것이 아래 "보안과 거버넌스" 절에서 다룰 McpInject 공격의 핵심 벡터다.
</Aside>
...
<Aside type="note" title="Agent OS Runtime 관점">
이번 주의 MCP는 "도구를 붙이는 방법"이 아니라 **capability boundary**로 이해해야 한다. Agent OS Runtime에서는 모든 tool call이 `allowed-tools` scope, input/output schema, `PreToolUse`/`PostToolUse` hook, `tool.invoke`/`tool.result` event를 통과한다. 이 기준은 이후 [Agent OS Runtime core architecture](/reference/agent-os-runtime/#7-layer-architecture)의 L1에서 다시 사용한다.
</Aside>
