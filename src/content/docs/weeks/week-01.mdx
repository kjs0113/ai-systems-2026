...
<CardGrid>
  ...
  <Card title="After (2025–2026)" icon="approve-check-circle">
    ...
    - SWE-bench Verified 성공률 79%대 (frontier reasoning model + agent harness)
  </Card>
</CardGrid>
...
**METR 연구 (2025.7, arXiv 2507.09089)**: 숙련 개발자 16명이 246개 실제 작업을 수행한 무작위 대조군 실험. 결과는 AI 도구 효과를 더 신중하게 해석해야 함을 보여준다.
...
- 2024년 초: 상위 시스템 ~15%
- 2025년 말: 50% 돌파
- 2026년 초: 79%대 frontier system 보고
...
핵심 오픈웨이트 코딩 모델:
- **Qwen3-Coder-Next** — 80B total / 3B 활성, 256K context, coding agent와 local development 지향
- **DeepSeek-V4-Pro / Flash** — 1.6T/284B MoE, 1M context, long-context reasoning과 agentic task 지향
- **GLM-5.1 / MiniMax-M2.7** — agentic engineering, terminal task, 장기 tool-use workflow 후보
...
15주 후, 여러분은 다음 시스템을 구현하고 검증하게 된다:
...
2. **SWE-bench 5% → 79%**: 3년 만에 에이전트 능력이 크게 향상됐다. 문제는 능력 자체보다 제어다.
...
6. **15주의 아크**: 거버넌스 → 인프라 → 루프 → 멀티에이전트 → MLOps → Ralphthon. 이 강의는 에이전틱 SDLC 파이프라인을 단계적으로 구축한다.
